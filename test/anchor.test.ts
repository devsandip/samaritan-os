...
import type { ExecutionAdapter } from "../src/types/index.js";

/** Records every filing attempt so a test can assert nothing was written. */
function spyAdapter(id: string): ExecutionAdapter & { calls: Record<string, unknown>[] } {
...
function harness(): Harness {
  ...
  // Swap the real Notion adapters for spies. The registry refuses duplicate
  // ids, so stub the registered instance rather than registering a second one.
  ...
  return {
    db,
    actionCenter: new ActionCenter({ db, capabilities, execution, routing }),
    notionDecision,
    notionInsight,
  };
}
